...
def run_code(codes, clock, base, inputs):
	...
	while codes[clock] != 99:
		...
		try:
			...
			to_idx = max(to_idx, code_val3 + base)
		except IndexError:
...
for key in seen.keys():
	...
	print(x, y)
	if x < 0 and x < -offset_x:
		offset_x = -x
	elif x > max_x:
		max_x = x

	if y < 0 and y < -offset_y:
		offset_y = -y
	elif y > max_y:
		max_y = y

print(offset_x, max_x)
print(offset_y, max_y)
...
for key, val in seen.items():
	new_x, new_y = list(map(int, key.split(',')))
	# print(key, f'{new_x+offset_x},{new_y+offset_y}')
	new_seen[f'{new_x+offset_x},{new_y+offset_y}'] = val
	del new_seen[key]
seen = new_seen
...
for y in range(max_y+1):
	...
	for x in range(max_x+1):
		try:
			map_created[y].append(seen[f'{x},{y}'])
		except KeyError:
			map_created[y].append(0)

for row in range(max_y, -1 ,-1):
	for col in range(max_x+1):
		print(digit[str(map_created[row][col])], end='')
	print()
